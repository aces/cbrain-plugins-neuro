...
class CbrainTask::Civet < ClusterTask
  ...
  def cluster_commands_single(arg_idx) #:nodoc:
    ...
    if params[:model].present?
      ...
      raise "Model is not valid for this CIVET version" if params[:model] == "ADNInl" && !@tool_config.is_at_least_version("1.1.12")
    end
    ...
    args += "-headheight #{params[:headheight]} "   if params[:headheight].present? && @tool_config.is_at_least_version("1.1.12")
    ...
    if mybool(params[:resample_surfaces])
      ...
      if @tool_config.is_at_least_version("1.1.11")
