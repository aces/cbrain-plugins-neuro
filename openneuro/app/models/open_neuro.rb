...
# not directly. It is a pair of a WorkGroup and a DataProvider.
# Right now the code is hardcoded with a DataladDataProvider
# which is not part of the core distribution of CBRAIN, so
# we don't use the class explicitely to prevent a plain
# CBRAIN from autoloading it.
class OpenNeuro
  ...
  def autoregister!
    ...
    registered = fis.each_with_index.map do |fi,idx|
      ...
      userfile = suggested_klass.new(
        :name             => fi.name,
        :size             => fi.size,  # can be nil for :directory
  ...
  def self.data_provider_builder(name, version, group_id)
    dp_class = 'DataladDataProvider' # not in CBRAIN codebase
    dp_class.constantize.where(
